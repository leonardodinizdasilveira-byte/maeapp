...
// Mantém a sessão ativa enquanto a aba estiver aberta (sem localStorage, que
// não funciona no preview). Em produção (Vercel/Bolt/seu servidor), troque as
// funções loadStore/saveStore por localStorage para persistir entre sessões.
const _store = { mgdf_loggedIn:false, mgdf_mae:{nome:"",email:"",celular:"",regiao:""}, mgdf_filhos:[] };
function loadStore(key, fallback){ return key in _store ? _store[key] : fallback; }
function saveStore(key, val){ _store[key] = val; }
function clearStore(){ _store.mgdf_loggedIn=false; _store.mgdf_mae={nome:"",email:"",celular:"",regiao:""}; _store.mgdf_filhos=[]; }
        ...
        <div style={{marginBottom:28}}>
          <div style={{display:"flex", alignItems:"center", gap:10, marginBottom:4}}>
            ...
            <div>
              ...
              <p style={{fontSize:11, color:"var(--lav)", fontWeight:700}}>DF</p>
            </div>
          </div>
          <p style={{fontSize:12, color:"#888", marginTop:8}}>Olá, {mae.nome.split(" ")[0]} 💜</p>
        </div>
...
function CampoInput({ icon, label, dica, erro, sufixo, children }) {
  ...
  return (
    <div>
      <label style={{display:"block", fontSize:12, fontWeight:600, color:"#6b7280", marginBottom:6, marginLeft:4, letterSpacing:0.2}}>{label}</label>
      <div
      ...
      {dica && !erro && <p style={{fontSize:11.5, color:"#9aa3a0", marginTop:5, marginLeft:6, lineHeight:1.4}}>{dica}</p>}
      {erro && <p style={{color:"#c0392b", fontSize:12, marginTop:5, marginLeft:6, fontWeight:600}}>⚠ {erro}</p>}
    </div>
  );
}

const inputBare = {width:"100%", border:"none", outline:"none", background:"transparent", fontSize:15, fontFamily:"inherit", color:"#222", padding:0};

function CampoTextarea({ icon, label, dica, cor="#3d9b7a", rows=4, value, onChange, placeholder }) {
  const [focado, setFocado] = useState(false);
  return (
    <div>
      <label style={{display:"flex", alignItems:"center", gap:6, fontSize:12, fontWeight:600, color:"#6b7280", marginBottom:6, marginLeft:4, letterSpacing:0.2}}>
      ...
      <div
        ...
        <textarea rows={rows} value={value} onChange={onChange} placeholder={placeholder}
          style={{width:"100%", border:"none", outline:"none", background:"transparent", fontSize:14, fontFamily:"inherit", color:"#222", resize:"vertical", lineHeight:1.6}}/>
      </div>
      {dica && <p style={{fontSize:11.5, color:"#9aa3a0", marginTop:5, marginLeft:6, lineHeight:1.4}}>{dica}</p>}
    </div>
  );
}
        ...
        {step === 1 && (
          <div>
            <div style={{marginBottom:22, padding:"0 4px"}}>
              ...
              <p style={{color:"#6b7280", fontSize:14, lineHeight:1.5}}>Vamos criar seu perfil. Seus dados ficam seguros e ajudam a personalizar os alertas.</p>
            </div>
        ...
        {step === 2 && (
          <div>
            <div style={{marginBottom:18, padding:"0 4px"}}>
              ...
              <p style={{color:"#6b7280", fontSize:14, lineHeight:1.5}}>Adicione um ou mais filhos. Você poderá gerenciar cada um separadamente.</p>
            </div>
              ...
              <div key={f.id} style={{background:"#fff", border:"1.5px solid #d8efe6", borderRadius:16, padding:"14px 16px", marginBottom:12, display:"flex", alignItems:"center", justifyContent:"space-between", boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
                <div style={{display:"flex", alignItems:"center", gap:12}}>
                  ...
                  <div>
                    ...
                    <p style={{fontSize:12, color:"#888", margin:0}}>{f.diagnosticos.join(", ") || "Diagnóstico a definir"}</p>
                  </div>
                </div>
            ...
            <div style={{background:"#fff", border:"1.5px dashed #b8e0d2", borderRadius:18, padding:"18px", marginBottom:16, boxShadow:"0 1px 3px rgba(0,0,0,0.03)"}}>
              <div style={{display:"flex", flexDirection:"column", gap:14}}>
                ...
                <div>
                  <p style={{fontSize:12, fontWeight:600, color:"#6b7280", marginBottom:10, marginLeft:4, letterSpacing:0.2}}>DIAGNÓSTICO(S)</p>
                  <div style={{display:"flex", flexWrap:"wrap", gap:8}}>
                    {DIAGNOSTICOS.map(d => {
                      const on = novoFilho.diagnosticos.includes(d);
                      return (
                        <button key={d} onClick={()=>toggleDiag(d)} style={{padding:"7px 14px", borderRadius:14, fontSize:13, fontWeight:700, cursor:"pointer", border:"1.5px solid", fontFamily:"inherit", transition:"all .2s", borderColor: on ? "#3d9b7a" : "#e5e7eb", background: on ? "#e8f5f0" : "#fafafa", color: on ? "#2f7d62" : "#9ca3af"}}>
                          {on && "✓ "}{d}
                        </button>
                      );
                    })}
    ...
    <div>
      <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:28, flexWrap:"wrap", gap:12}}>
        <div>
          ...
          <p style={{color:"#888", fontSize:14}}>{hoje.toLocaleDateString("pt-BR",{weekday:"long",day:"numeric",month:"long"})}</p>
        </div>
        <div style={{display:"flex", alignItems:"center", gap:10, flexWrap:"wrap"}}>
          ...
          <button onClick={fazerLogout} style={{display:"flex", alignItems:"center", gap:6, background:"#fff0ef", color:"#c0392b", border:"1.5px solid #fcc", borderRadius:12, padding:"8px 16px", fontWeight:700, cursor:"pointer", fontSize:13, fontFamily:"inherit"}}>
            <LogOut size={15}/> Sair
          </button>
        </div>
      </div>
        ...
        <>
          ...
          <div className="card" style={{background:"linear-gradient(135deg,#e8f5f0,#f0eaf8)", marginBottom:20, display:"flex", alignItems:"center", gap:16, flexWrap:"wrap"}}>
            ...
            <div style={{flex:1}}>
              ...
              <p style={{margin:"2px 0", color:"#666", fontSize:14}}>{filho.idade} anos • {filho.diagnosticos.join(", ")}</p>
            ...
            <div style={{background:"#e8f5f0", border:"1.5px solid #b8e0d2", borderRadius:12, padding:"10px 16px", minWidth:200}}>
              <div style={{display:"flex", alignItems:"center", gap:6, marginBottom:4}}>
                ...
                <span style={{fontSize:11, fontWeight:800, color:"#3d9b7a"}}>STATUS DO SISTEMA</span>
              </div>
              <p style={{fontSize:12, color:"#444", margin:0, lineHeight:1.5}}>✅ Gatilhos ativos. Alertas via WhatsApp</p>
              <p style={{fontSize:12, fontWeight:700, color:"#3d9b7a", margin:"2px 0 0"}}>{mae.celular}</p>
            </div>
          </div>
              ...
              {[
                ...
                <div key={item.label} style={{display:"flex", alignItems:"center", gap:12, background:"#fafbfa", border:"1px solid #f0f0f0", borderRadius:14, padding:"12px 14px"}}>
                  ...
                  <div style={{minWidth:0}}>
                    <p style={{fontSize:10.5, fontWeight:600, color:"#9aa3a0", margin:0, letterSpacing:0.4}}>{item.label}</p>
          ...
          <div style={{display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:16, marginBottom:24}}>
            <div className="card" style={{borderLeft:"4px solid #3d9b7a"}}>
              ...
              {remedios.length === 0 ? <p style={{color:"#aaa", fontSize:13}}>Nenhum cadastrado ainda</p> : remedios.slice(0,3).map(r=>(
                <div key={r.id} style={{display:"flex", justifyContent:"space-between", alignItems:"center", padding:"6px 0", borderBottom:"1px solid #f0f0f0"}}>
                  ...
                  <span className="tag-mint" style={{fontSize:11, padding:"2px 8px", borderRadius:10, background:"var(--mint-light)", color:"var(--mint)", fontWeight:700}}>{r.horario}</span>
                </div>
              ))}
            ...
            <div className="card" style={{borderLeft:"4px solid #7c5cbf"}}>
              ...
              {exames.length === 0 ? <p style={{color:"#aaa", fontSize:13}}>Nenhum exame agendado</p> : exames.slice(0,2).map(e=>(
                <div key={e.id} style={{padding:"6px 0", borderBottom:"1px solid #f0f0f0"}}>
                  <p style={{fontWeight:600, fontSize:13, margin:0}}>{e.titulo}</p>
                  <p style={{fontSize:12, color:"#888", margin:"2px 0 0"}}>{e.data} às {e.hora} • {e.local}</p>
                </div>
              ))}
              <button className="btn-outline" onClick={()=>setTela("agenda")} style={{width:"100%",marginTop:10,padding:"6px",fontSize:13,borderColor:"#7c5cbf",color:"#7c5cbf"}}>Gerenciar Exames</button>
            </div>
          </div>
        </>
      )}
        ...
        <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16, flexWrap:"wrap", gap:10}}>
          <div style={{display:"flex", alignItems:"center", gap:10}}>
            ...
            <div>
              ...
              <p style={{fontSize:12, color:"#888", margin:0}}>Informações oficiais para {mae.regiao}</p>
            </div>
          </div>
          {!canalCadastrado ? (
            ...
            <span style={{background:"#e8f5f0", color:"#3d9b7a", borderRadius:10, padding:"6px 12px", fontSize:12, fontWeight:700}}>✅ Canal ativo: {mae.celular}</span>
          )}
        </div>
          ...
          {GDF_ALERTS.map(a=>(
            <div key={a.id} style={{display:"flex", gap:12, padding:"12px 14px", background:"#f8fcfa", borderRadius:12, border:"1px solid #e8f0ea", alignItems:"flex-start"}}>
              ...
              <div style={{flex:1}}>
                ...
                <p style={{fontSize:13, color:"#555", margin:0, lineHeight:1.5}}>{a.msg}</p>
              </div>
              <span style={{fontSize:11, color:"#aaa", flexShrink:0, marginTop:2}}>{a.data}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:12}}>
        {[
          ...
          <button key={a.tela} onClick={()=>setTela(a.tela)} style={{background:a.color, border:`1.5px solid ${a.border}`, borderRadius:14, padding:"16px 12px", cursor:"pointer", textAlign:"center", fontFamily:"inherit", transition:"transform .15s"}}>
            ...
            <p style={{fontWeight:700, fontSize:13, color:a.text, margin:0}}>{a.label}</p>
          </button>
        ))}
      </div>
    </div>
...
function TelaAgenda({remedios,setRemedios,exames,setExames,novoRemedio,setNovoRemedio,novoExame,setNovoExame,tocarAlarme,filho}) {
  ...
  return (
    <div>
      <h1 style={{fontWeight:800,fontSize:24,color:"#222",marginBottom:4}}>Agenda de Saúde</h1>
      <p style={{color:"#888",fontSize:14,marginBottom:24}}>{filho ? `Para: ${filho.nome}` : "Configure seus filhos no dashboard"}</p>
      ...
      <div className="card" style={{marginBottom:24}}>
        ...
        <div style={{background:"#f8fcfa",borderRadius:18,padding:18,marginBottom:20,border:"1px solid #e0ede6"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div>
              <p style={{fontSize:12,fontWeight:600,color:"#6b7280",marginBottom:10,marginLeft:4,letterSpacing:0.2}}>DIAS DA SEMANA</p>
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                {DIAS.map(d=>{
                  const on = novoRemedio.dias.includes(d);
                  return <button key={d} onClick={()=>toggleDia(d)} style={{padding:"7px 14px",borderRadius:14,fontSize:13,fontWeight:700,cursor:"pointer",border:"1.5px solid",fontFamily:"inherit",transition:"all .2s",borderColor:on?"#3d9b7a":"#e5e7eb",background:on?"#e8f5f0":"#fafafa",color:on?"#2f7d62":"#9ca3af"}}>{d}</button>;
                })}
              </div>
            </div>
        ...
        {/* Lista */}
        {remedios.length === 0 ? <p style={{color:"#aaa",textAlign:"center",padding:20}}>Nenhum medicamento cadastrado.</p> :
          remedios.map(r=>(
            <div key={r.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",background:r.ministrado?"#e8f5f0":"#fff",borderRadius:12,marginBottom:8,border:`1.5px solid ${r.ministrado?"#b8e0d2":"#eee"}`}}>
              <div style={{flex:1}}>
                ...
                <p style={{fontSize:12,color:"#888",margin:"2px 0 0"}}>{r.dosagem} • {r.dias.join(", ")} • {r.horario}</p>
              </div>
              {!r.ministrado && <>
                <button onClick={()=>tocarAlarme(`⏰ Hora do remédio! ${r.nome} — ${r.dosagem}`)} style={{background:"#fff3e0",border:"none",borderRadius:8,padding:"6px 10px",cursor:"pointer",fontSize:12,fontWeight:700,color:"#e07b39",fontFamily:"inherit"}} title="Simular alarme"><Volume2 size={14}/></button>
                <button onClick={()=>marcarMinistrado(r.id)} style={{background:"#e8f5f0",border:"none",borderRadius:8,padding:"6px 10px",cursor:"pointer",fontSize:12,fontWeight:700,color:"#3d9b7a",fontFamily:"inherit"}}><Check size={14}/></button>
              </>}
      ...
      <div className="card">
        ...
        {exames.length === 0 ? <p style={{color:"#aaa",textAlign:"center",padding:20}}>Nenhum exame agendado.</p> :
          exames.map(e=>(
            <div key={e.id} style={{padding:"12px 14px",background:"#faf8ff",borderRadius:12,marginBottom:8,border:"1.5px solid #e8d8f4"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div style={{flex:1}}>
                  ...
                  <p style={{fontSize:12,color:"#888",margin:"2px 0 0"}}>{e.data} às {e.hora} • {e.local}</p>
                  {e.notas && <p style={{fontSize:12,color:"#e07b39",margin:"4px 0 0",background:"#fff8f0",padding:"4px 8px",borderRadius:6}}>⚠️ {e.notas}</p>}
                </div>
...
function TelaRequerimentos({mae,filho,filhos,filhoSelecionado,setFilhoSelecionado,reqSelecionado,setReqSelecionado,textoCopied,setTextoCopied,docSalvo,salvarRequerimento,gerarTexto}) {
  ...
  return (
    <div>
      ...
      <p style={{color:"#888",fontSize:14,marginBottom:20}}>9 modelos com fundamentação jurídica real • Clique para gerar</p>

      {filhos.length > 1 && (
        <div style={{marginBottom:20}}>
          <label style={{fontSize:13,fontWeight:700,color:"#555",display:"block",marginBottom:6}}>Filho(a) para o documento:</label>
        ...
        <div>
          {cats.map(cat=>(
            <div key={cat} style={{marginBottom:16}}>
              <p style={{fontSize:11,fontWeight:800,color:"#888",letterSpacing:1,marginBottom:8,textTransform:"uppercase"}}>{cat}</p>
              {REQUERIMENTOS.filter(r=>r.categoria===cat).map(r=>(
                <button key={r.id} onClick={()=>setReqSelecionado(r)} style={{width:"100%",textAlign:"left",padding:"10px 14px",borderRadius:12,marginBottom:6,cursor:"pointer",fontFamily:"inherit",border:`1.5px solid ${reqSelecionado?.id===r.id?"#3d9b7a":"#eee"}`,background:reqSelecionado?.id===r.id?"#e8f5f0":"white",transition:"all .15s"}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <span style={{fontSize:13,fontWeight:800,color:"#3d9b7a",flexShrink:0}}>#{r.id}</span>
                    <span style={{fontSize:13,fontWeight:600,color:reqSelecionado?.id===r.id?"#3d9b7a":"#333",lineHeight:1.3}}>{r.titulo}</span>
                  </div>
                </button>
              ))}
            </div>
            ...
            <>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:10}}>
                <div>
                  ...
                  <p style={{fontSize:12,color:"#7c5cbf",fontWeight:600}}>{reqSelecionado.fundamentacao}</p>
                </div>
                <div style={{display:"flex",gap:8}}>
                  <button className="btn-outline" onClick={copiar} style={{padding:"8px 14px",fontSize:13,display:"flex",alignItems:"center",gap:6}}>
                    {textoCopied ? <><Check size={14} color="#3d9b7a"/>Copiado!</> : <><Copy size={14}/>Copiar</>}
                  </button>
                  <button className="btn-mint" onClick={salvarRequerimento} style={{padding:"8px 14px",fontSize:13,display:"flex",alignItems:"center",gap:6}}>
                    {docSalvo ? <><Check size={14}/>Salvo!</> : <><Save size={14}/>Salvar no Arquivo</>}
                  </button>
                </div>
              </div>
              <div style={{background:"#f9f9f9",borderRadius:12,padding:"16px 20px",flex:1,border:"1px solid #eee",fontFamily:"'Courier New',monospace",fontSize:13,lineHeight:1.8,whiteSpace:"pre-wrap",overflowY:"auto",maxHeight:520,color:"#333"}}>
                {texto}
              </div>
            </>
          )}
...
function TelaDocumentos({filho,filhoSelecionado,getDocFilho,setDocFilho,gaveta,setGaveta,uploadProgress,cameraInputRef,galeriaInputRef,handleUpload}) {
  ...
  return (
    <div>
      ...
      <p style={{color:"#888",fontSize:14,marginBottom:20}}>{filho ? `Pasta de ${filho.nome}` : "Selecione um filho no dashboard"}</p>
      ...
      {uploading.map(([key,prog])=>(
        <div key={key} style={{background:"#f8fcfa",borderRadius:12,padding:"14px 16px",marginBottom:12,border:"1px solid #e0ede6"}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
            <span style={{fontSize:13,fontWeight:700,color:"#3d9b7a"}}>A carregar ficheiro...</span>
            <span style={{fontSize:13,fontWeight:700,color:"#3d9b7a"}}>{Math.round(prog)}%</span>
          </div>
      ...
      <div className="card">
        <h3 style={{fontWeight:800,fontSize:16,margin:"0 0 16px",color:"#222"}}>
          ...
          <span style={{fontSize:13,fontWeight:600,color:"#aaa",marginLeft:8}}>({docs.length} arquivo{docs.length!==1?"s":""})</span>
        </h3>
        {docs.length === 0 ? (
          ...
          <div key={doc.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:"1px solid #f0f0f0"}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <p style={{fontWeight:700,fontSize:13,margin:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{doc.nome}</p>
              <p style={{fontSize:12,color:"#aaa",margin:"2px 0 0"}}>{doc.data} • {doc.tamanho}</p>
              {doc.conteudo && <p style={{fontSize:11,color:"#7c5cbf",margin:"2px 0 0",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{doc.conteudo.substring(0,60)}...</p>}
            </div>
...
function TelaCuidador({filho,filhoSelecionado,cuidador,setCuidador,mae,gerarMsg,enviarWhatsApp,msgCopiada,setMsgCopiada}) {
  ...
  return (
    <div>
      ...
      <p style={{color:"#888",fontSize:14,marginBottom:24}}>Envie as instruções do dia para quem cuida de {filho?.nome || "sua criança"}</p>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:20,alignItems:"start"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div className="card">
            <h3 style={{fontWeight:800,fontSize:15,margin:"0 0 16px",color:"#888",display:"flex",alignItems:"center",gap:8}}>📌 Observações Especiais</h3>
        ...
        <div>
          <div className="card" style={{marginBottom:16}}>
            ...
            <div style={{background:"#f0f8f4",borderRadius:12,padding:16,border:"1px solid #c8e6d8",fontFamily:"'Courier New',monospace",fontSize:12,lineHeight:1.8,whiteSpace:"pre-wrap",maxHeight:360,overflowY:"auto",color:"#333"}}>
              {gerarMsg()}
            </div>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
...
function TelaDireitos({direitoAberto,setDireitoAberto,checklistFeito,setChecklistFeito,setTela}) {
  ...
  return (
    <div>
      ...
      <p style={{color:"#888",fontSize:14,marginBottom:24}}>Focado no Distrito Federal • Checklists interativos passo a passo</p>
              ...
              <div key={d.id} className="card" style={{padding:0,overflow:"hidden"}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
                    <div style={{display:"flex",alignItems:"center",gap:12}}>
                      ...
                      <div>
                        ...
                        <p style={{fontSize:12,color:"#888",margin:"2px 0 0"}}>{done}/{d.checklist.length} etapas concluídas</p>
                      </div>
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                ...
                {open && (
                  <div style={{borderTop:"1px solid #f0f0f0",padding:"16px 20px"}}>
                    <p style={{fontSize:13,fontWeight:700,color:"#555",marginBottom:12}}>Passo a passo:</p>
                    ...
                    <div style={{background:"#fff8e6",border:"1.5px solid #ffd87a",borderRadius:12,padding:"12px 16px",marginTop:16}}>
                      <p style={{fontWeight:800,fontSize:13,color:"#b8860b",margin:"0 0 4px"}}>⚠️ Em caso de Negativa ou Recusa:</p>
                      <p style={{fontSize:13,color:"#555",margin:0,lineHeight:1.6}}>{d.negativa}</p>
                    </div>
                    <button className="btn-outline" onClick={()=>setTela("requerimentos")} style={{marginTop:14,fontSize:13,display:"flex",alignItems:"center",gap:6}}>
      ...
      <div>
        ...
        <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
          <button onClick={()=>setFiltroRede("")} style={{padding:"6px 14px",borderRadius:20,fontSize:12,fontWeight:700,cursor:"pointer",border:"1.5px solid",fontFamily:"inherit",borderColor:filtroRede===""?"#3d9b7a":"#ddd",background:filtroRede===""?"#e8f5f0":"white",color:filtroRede===""?"#3d9b7a":"#888"}}>Todos</button>
          {TIPOS.map(t=>(
            <button key={t} onClick={()=>setFiltroRede(t===filtroRede?"":t)} style={{padding:"6px 14px",borderRadius:20,fontSize:12,fontWeight:700,cursor:"pointer",border:"1.5px solid",fontFamily:"inherit",borderColor:filtroRede===t?"#7c5cbf":"#ddd",background:filtroRede===t?"#f0eaf8":"white",color:filtroRede===t?"#7c5cbf":"#888"}}>{t}</button>
          ))}
        </div>
            ...
            return (
              <div key={r.nome} style={{background:col.bg,border:`1.5px solid ${col.border}`,borderRadius:14,padding:"14px 16px"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                  <span style={{fontSize:11,fontWeight:800,color:col.tag,background:"white",borderRadius:8,padding:"2px 8px",border:`1px solid ${col.border}`}}>{r.tipo}</span>
                  <span style={{fontSize:11,color:"#aaa"}}>{r.regiao}</span>
                </div>
                <p style={{fontWeight:700,fontSize:14,margin:"0 0 4px",color:"#222"}}>{r.nome}</p>
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}>
                  <MapPin size={11} color="#aaa"/>
                  <p style={{fontSize:12,color:"#666",margin:0}}>{r.endereco}</p>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:6}}>
                  <Phone size={11} color="#aaa"/>
                  <p style={{fontSize:12,fontWeight:700,color:col.tag,margin:0}}>{r.tel}</p>
                </div>
              </div>
            );
          })}
